...
static inline bool as_bool_cstr(const char* c) {
  return c && std::strtol(c, nullptr, 10) != 0;
}
                             ...
                             std::string& err)
{
  ...
  while ((row = mysql_fetch_row(res))) {
    ...
    m.start_point_z = row[idx++] ? static_cast<float>(std::strtof(row[idx-1], nullptr)) : 0.f;
    ...
    m.end_point_z   = row[idx++] ? static_cast<float>(std::strtof(row[idx-1], nullptr)) : 0.f;
    ...
    m.u_turn_allowed   = as_bool_cstr(row[idx++]);
    m.slope_alert      = as_bool_cstr(row[idx++]);
    m.vegetation_alert = as_bool_cstr(row[idx++]);
    m.offroad_alert    = as_bool_cstr(row[idx++]);
...
bool DBMySQL::fetch_pois(const DBFilter& f,
                         ...
                         std::string& err)
{
  ...
  while ((row = mysql_fetch_row(res))) {
    int idx = 0;
    hive_interface2::msg::Poi m;

    m.enterprise_full_id_str   = row[idx++] ? row[idx-1] : "";
    m.deployment_full_id_str   = row[idx++] ? row[idx-1] : "";
    m.slam_session_full_id_str = row[idx++] ? row[idx-1] : "";

    m.free_zone_full_id_str    = row[idx++] ? row[idx-1] : "";
    m.is_in_free_zone          = row[idx++] ? static_cast<int32_t>(std::strtol(row[idx-1], nullptr, 10)) : 0;

    m.poi_full_id_str          = row[idx++] ? row[idx-1] : "";
    m.poi_id                   = row[idx++] ? static_cast<int32_t>(std::strtol(row[idx-1], nullptr, 10)) : 0;

    m.x   = row[idx++] ? static_cast<float>(std::strtof(row[idx-1], nullptr)) : 0.f;
    m.y   = row[idx++] ? static_cast<float>(std::strtof(row[idx-1], nullptr)) : 0.f;
    m.z   = row[idx++] ? static_cast<float>(std::strtof(row[idx-1], nullptr)) : 0.f;
    m.yaw = row[idx++] ? static_cast<float>(std::strtof(row[idx-1], nullptr)) : 0.f;

    m.parking_mode      = row[idx++] ? static_cast<int32_t>(std::strtol(row[idx-1], nullptr, 10)) : 0;
    m.is_available      = row[idx++] ? static_cast<int32_t>(std::strtol(row[idx-1], nullptr, 10)) : 0;
    m.charging_mode     = row[idx++] ? static_cast<int32_t>(std::strtol(row[idx-1], nullptr, 10)) : 0;
    m.is_long_wait_zone = row[idx++] ? static_cast<int32_t>(std::strtol(row[idx-1], nullptr, 10)) : 0;
...
bool DBMySQL::fetch_free_zones(const DBFilter& f,
                               ...
                               std::string& err)
{
  ...
  while ((row = mysql_fetch_row(res))) {
    int idx = 0;
    hive_interface2::msg::FreeZone m;
    ...
    m.free_zone_full_id_str     = row[idx++] ? row[idx-1] : "";
    m.free_zone_id              = row[idx++] ? row[idx-1] : "";

    m.is_parking_available      = row[idx++] ? static_cast<int32_t>(std::strtol(row[idx-1], nullptr, 10)) : 0;
    m.is_offpoi_parking_allowed = row[idx++] ? static_cast<int32_t>(std::strtol(row[idx-1], nullptr, 10)) : 0;
    m.is_long_wait_zone         = row[idx++] ? static_cast<int32_t>(std::strtol(row[idx-1], nullptr, 10)) : 0;

    m.zone_polygon_points_json  = row[idx++] ? row[idx-1] : "";
