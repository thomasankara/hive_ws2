// generated from rosidl_typesupport_c/resource/idl__type_support.cpp.em
// with input from dwb_msgs:srv/GenerateTwists.idl
// generated code does not contain a copyright notice

#include "cstddef"
#include "rosidl_runtime_c/message_type_support_struct.h"
#include "dwb_msgs/srv/detail/generate_twists__struct.h"
#include "dwb_msgs/srv/detail/generate_twists__type_support.h"
#include "dwb_msgs/srv/detail/generate_twists__functions.h"
#include "rosidl_typesupport_c/identifier.h"
#include "rosidl_typesupport_c/message_type_support_dispatch.h"
#include "rosidl_typesupport_c/type_support_map.h"
#include "rosidl_typesupport_c/visibility_control.h"
#include "rosidl_typesupport_interface/macros.h"

namespace dwb_msgs
{

namespace srv
{

namespace rosidl_typesupport_c
{

typedef struct _GenerateTwists_Request_type_support_ids_t
{
  const char * typesupport_identifier[2];
} _GenerateTwists_Request_type_support_ids_t;

static const _GenerateTwists_Request_type_support_ids_t _GenerateTwists_Request_message_typesupport_ids = {
  {
    "rosidl_typesupport_fastrtps_c",  // ::rosidl_typesupport_fastrtps_c::typesupport_identifier,
    "rosidl_typesupport_introspection_c",  // ::rosidl_typesupport_introspection_c::typesupport_identifier,
  }
};

typedef struct _GenerateTwists_Request_type_support_symbol_names_t
{
  const char * symbol_name[2];
} _GenerateTwists_Request_type_support_symbol_names_t;

#define STRINGIFY_(s) #s
#define STRINGIFY(s) STRINGIFY_(s)

static const _GenerateTwists_Request_type_support_symbol_names_t _GenerateTwists_Request_message_typesupport_symbol_names = {
  {
    STRINGIFY(ROSIDL_TYPESUPPORT_INTERFACE__MESSAGE_SYMBOL_NAME(rosidl_typesupport_fastrtps_c, dwb_msgs, srv, GenerateTwists_Request)),
    STRINGIFY(ROSIDL_TYPESUPPORT_INTERFACE__MESSAGE_SYMBOL_NAME(rosidl_typesupport_introspection_c, dwb_msgs, srv, GenerateTwists_Request)),
  }
};

typedef struct _GenerateTwists_Request_type_support_data_t
{
  void * data[2];
} _GenerateTwists_Request_type_support_data_t;

static _GenerateTwists_Request_type_support_data_t _GenerateTwists_Request_message_typesupport_data = {
  {
    0,  // will store the shared library later
    0,  // will store the shared library later
  }
};

static const type_support_map_t _GenerateTwists_Request_message_typesupport_map = {
  2,
  "dwb_msgs",
  &_GenerateTwists_Request_message_typesupport_ids.typesupport_identifier[0],
  &_GenerateTwists_Request_message_typesupport_symbol_names.symbol_name[0],
  &_GenerateTwists_Request_message_typesupport_data.data[0],
};

static const rosidl_message_type_support_t GenerateTwists_Request_message_type_support_handle = {
  rosidl_typesupport_c__typesupport_identifier,
  reinterpret_cast<const type_support_map_t *>(&_GenerateTwists_Request_message_typesupport_map),
  rosidl_typesupport_c__get_message_typesupport_handle_function,
  &dwb_msgs__srv__GenerateTwists_Request__get_type_hash,
  &dwb_msgs__srv__GenerateTwists_Request__get_type_description,
  &dwb_msgs__srv__GenerateTwists_Request__get_type_description_sources,
};

}  // namespace rosidl_typesupport_c

}  // namespace srv

}  // namespace dwb_msgs

#ifdef __cplusplus
extern "C"
{
#endif

const rosidl_message_type_support_t *
ROSIDL_TYPESUPPORT_INTERFACE__MESSAGE_SYMBOL_NAME(rosidl_typesupport_c, dwb_msgs, srv, GenerateTwists_Request)() {
  return &::dwb_msgs::srv::rosidl_typesupport_c::GenerateTwists_Request_message_type_support_handle;
}

#ifdef __cplusplus
}
#endif

// already included above
// #include "cstddef"
// already included above
// #include "rosidl_runtime_c/message_type_support_struct.h"
// already included above
// #include "dwb_msgs/srv/detail/generate_twists__struct.h"
// already included above
// #include "dwb_msgs/srv/detail/generate_twists__type_support.h"
// already included above
// #include "dwb_msgs/srv/detail/generate_twists__functions.h"
// already included above
// #include "rosidl_typesupport_c/identifier.h"
// already included above
// #include "rosidl_typesupport_c/message_type_support_dispatch.h"
// already included above
// #include "rosidl_typesupport_c/type_support_map.h"
// already included above
// #include "rosidl_typesupport_c/visibility_control.h"
// already included above
// #include "rosidl_typesupport_interface/macros.h"

namespace dwb_msgs
{

namespace srv
{

namespace rosidl_typesupport_c
{

typedef struct _GenerateTwists_Response_type_support_ids_t
{
  const char * typesupport_identifier[2];
} _GenerateTwists_Response_type_support_ids_t;

static const _GenerateTwists_Response_type_support_ids_t _GenerateTwists_Response_message_typesupport_ids = {
  {
    "rosidl_typesupport_fastrtps_c",  // ::rosidl_typesupport_fastrtps_c::typesupport_identifier,
    "rosidl_typesupport_introspection_c",  // ::rosidl_typesupport_introspection_c::typesupport_identifier,
  }
};

typedef struct _GenerateTwists_Response_type_support_symbol_names_t
{
  const char * symbol_name[2];
} _GenerateTwists_Response_type_support_symbol_names_t;

#define STRINGIFY_(s) #s
#define STRINGIFY(s) STRINGIFY_(s)

static const _GenerateTwists_Response_type_support_symbol_names_t _GenerateTwists_Response_message_typesupport_symbol_names = {
  {
    STRINGIFY(ROSIDL_TYPESUPPORT_INTERFACE__MESSAGE_SYMBOL_NAME(rosidl_typesupport_fastrtps_c, dwb_msgs, srv, GenerateTwists_Response)),
    STRINGIFY(ROSIDL_TYPESUPPORT_INTERFACE__MESSAGE_SYMBOL_NAME(rosidl_typesupport_introspection_c, dwb_msgs, srv, GenerateTwists_Response)),
  }
};

typedef struct _GenerateTwists_Response_type_support_data_t
{
  void * data[2];
} _GenerateTwists_Response_type_support_data_t;

static _GenerateTwists_Response_type_support_data_t _GenerateTwists_Response_message_typesupport_data = {
  {
    0,  // will store the shared library later
    0,  // will store the shared library later
  }
};

static const type_support_map_t _GenerateTwists_Response_message_typesupport_map = {
  2,
  "dwb_msgs",
  &_GenerateTwists_Response_message_typesupport_ids.typesupport_identifier[0],
  &_GenerateTwists_Response_message_typesupport_symbol_names.symbol_name[0],
  &_GenerateTwists_Response_message_typesupport_data.data[0],
};

static const rosidl_message_type_support_t GenerateTwists_Response_message_type_support_handle = {
  rosidl_typesupport_c__typesupport_identifier,
  reinterpret_cast<const type_support_map_t *>(&_GenerateTwists_Response_message_typesupport_map),
  rosidl_typesupport_c__get_message_typesupport_handle_function,
  &dwb_msgs__srv__GenerateTwists_Response__get_type_hash,
  &dwb_msgs__srv__GenerateTwists_Response__get_type_description,
  &dwb_msgs__srv__GenerateTwists_Response__get_type_description_sources,
};

}  // namespace rosidl_typesupport_c

}  // namespace srv

}  // namespace dwb_msgs

#ifdef __cplusplus
extern "C"
{
#endif

const rosidl_message_type_support_t *
ROSIDL_TYPESUPPORT_INTERFACE__MESSAGE_SYMBOL_NAME(rosidl_typesupport_c, dwb_msgs, srv, GenerateTwists_Response)() {
  return &::dwb_msgs::srv::rosidl_typesupport_c::GenerateTwists_Response_message_type_support_handle;
}

#ifdef __cplusplus
}
#endif

// already included above
// #include "cstddef"
// already included above
// #include "rosidl_runtime_c/message_type_support_struct.h"
// already included above
// #include "dwb_msgs/srv/detail/generate_twists__struct.h"
// already included above
// #include "dwb_msgs/srv/detail/generate_twists__type_support.h"
// already included above
// #include "dwb_msgs/srv/detail/generate_twists__functions.h"
// already included above
// #include "rosidl_typesupport_c/identifier.h"
// already included above
// #include "rosidl_typesupport_c/message_type_support_dispatch.h"
// already included above
// #include "rosidl_typesupport_c/type_support_map.h"
// already included above
// #include "rosidl_typesupport_c/visibility_control.h"
// already included above
// #include "rosidl_typesupport_interface/macros.h"

namespace dwb_msgs
{

namespace srv
{

namespace rosidl_typesupport_c
{

typedef struct _GenerateTwists_Event_type_support_ids_t
{
  const char * typesupport_identifier[2];
} _GenerateTwists_Event_type_support_ids_t;

static const _GenerateTwists_Event_type_support_ids_t _GenerateTwists_Event_message_typesupport_ids = {
  {
    "rosidl_typesupport_fastrtps_c",  // ::rosidl_typesupport_fastrtps_c::typesupport_identifier,
    "rosidl_typesupport_introspection_c",  // ::rosidl_typesupport_introspection_c::typesupport_identifier,
  }
};

typedef struct _GenerateTwists_Event_type_support_symbol_names_t
{
  const char * symbol_name[2];
} _GenerateTwists_Event_type_support_symbol_names_t;

#define STRINGIFY_(s) #s
#define STRINGIFY(s) STRINGIFY_(s)

static const _GenerateTwists_Event_type_support_symbol_names_t _GenerateTwists_Event_message_typesupport_symbol_names = {
  {
    STRINGIFY(ROSIDL_TYPESUPPORT_INTERFACE__MESSAGE_SYMBOL_NAME(rosidl_typesupport_fastrtps_c, dwb_msgs, srv, GenerateTwists_Event)),
    STRINGIFY(ROSIDL_TYPESUPPORT_INTERFACE__MESSAGE_SYMBOL_NAME(rosidl_typesupport_introspection_c, dwb_msgs, srv, GenerateTwists_Event)),
  }
};

typedef struct _GenerateTwists_Event_type_support_data_t
{
  void * data[2];
} _GenerateTwists_Event_type_support_data_t;

static _GenerateTwists_Event_type_support_data_t _GenerateTwists_Event_message_typesupport_data = {
  {
    0,  // will store the shared library later
    0,  // will store the shared library later
  }
};

static const type_support_map_t _GenerateTwists_Event_message_typesupport_map = {
  2,
  "dwb_msgs",
  &_GenerateTwists_Event_message_typesupport_ids.typesupport_identifier[0],
  &_GenerateTwists_Event_message_typesupport_symbol_names.symbol_name[0],
  &_GenerateTwists_Event_message_typesupport_data.data[0],
};

static const rosidl_message_type_support_t GenerateTwists_Event_message_type_support_handle = {
  rosidl_typesupport_c__typesupport_identifier,
  reinterpret_cast<const type_support_map_t *>(&_GenerateTwists_Event_message_typesupport_map),
  rosidl_typesupport_c__get_message_typesupport_handle_function,
  &dwb_msgs__srv__GenerateTwists_Event__get_type_hash,
  &dwb_msgs__srv__GenerateTwists_Event__get_type_description,
  &dwb_msgs__srv__GenerateTwists_Event__get_type_description_sources,
};

}  // namespace rosidl_typesupport_c

}  // namespace srv

}  // namespace dwb_msgs

#ifdef __cplusplus
extern "C"
{
#endif

const rosidl_message_type_support_t *
ROSIDL_TYPESUPPORT_INTERFACE__MESSAGE_SYMBOL_NAME(rosidl_typesupport_c, dwb_msgs, srv, GenerateTwists_Event)() {
  return &::dwb_msgs::srv::rosidl_typesupport_c::GenerateTwists_Event_message_type_support_handle;
}

#ifdef __cplusplus
}
#endif

// already included above
// #include "cstddef"
#include "rosidl_runtime_c/service_type_support_struct.h"
// already included above
// #include "dwb_msgs/srv/detail/generate_twists__type_support.h"
// already included above
// #include "rosidl_typesupport_c/identifier.h"
#include "rosidl_typesupport_c/service_type_support_dispatch.h"
// already included above
// #include "rosidl_typesupport_c/type_support_map.h"
// already included above
// #include "rosidl_typesupport_interface/macros.h"
#include "service_msgs/msg/service_event_info.h"
#include "builtin_interfaces/msg/time.h"

namespace dwb_msgs
{

namespace srv
{

namespace rosidl_typesupport_c
{
typedef struct _GenerateTwists_type_support_ids_t
{
  const char * typesupport_identifier[2];
} _GenerateTwists_type_support_ids_t;

static const _GenerateTwists_type_support_ids_t _GenerateTwists_service_typesupport_ids = {
  {
    "rosidl_typesupport_fastrtps_c",  // ::rosidl_typesupport_fastrtps_c::typesupport_identifier,
    "rosidl_typesupport_introspection_c",  // ::rosidl_typesupport_introspection_c::typesupport_identifier,
  }
};

typedef struct _GenerateTwists_type_support_symbol_names_t
{
  const char * symbol_name[2];
} _GenerateTwists_type_support_symbol_names_t;

#define STRINGIFY_(s) #s
#define STRINGIFY(s) STRINGIFY_(s)

static const _GenerateTwists_type_support_symbol_names_t _GenerateTwists_service_typesupport_symbol_names = {
  {
    STRINGIFY(ROSIDL_TYPESUPPORT_INTERFACE__SERVICE_SYMBOL_NAME(rosidl_typesupport_fastrtps_c, dwb_msgs, srv, GenerateTwists)),
    STRINGIFY(ROSIDL_TYPESUPPORT_INTERFACE__SERVICE_SYMBOL_NAME(rosidl_typesupport_introspection_c, dwb_msgs, srv, GenerateTwists)),
  }
};

typedef struct _GenerateTwists_type_support_data_t
{
  void * data[2];
} _GenerateTwists_type_support_data_t;

static _GenerateTwists_type_support_data_t _GenerateTwists_service_typesupport_data = {
  {
    0,  // will store the shared library later
    0,  // will store the shared library later
  }
};

static const type_support_map_t _GenerateTwists_service_typesupport_map = {
  2,
  "dwb_msgs",
  &_GenerateTwists_service_typesupport_ids.typesupport_identifier[0],
  &_GenerateTwists_service_typesupport_symbol_names.symbol_name[0],
  &_GenerateTwists_service_typesupport_data.data[0],
};

static const rosidl_service_type_support_t GenerateTwists_service_type_support_handle = {
  rosidl_typesupport_c__typesupport_identifier,
  reinterpret_cast<const type_support_map_t *>(&_GenerateTwists_service_typesupport_map),
  rosidl_typesupport_c__get_service_typesupport_handle_function,
  &GenerateTwists_Request_message_type_support_handle,
  &GenerateTwists_Response_message_type_support_handle,
  &GenerateTwists_Event_message_type_support_handle,
  ROSIDL_TYPESUPPORT_INTERFACE__SERVICE_CREATE_EVENT_MESSAGE_SYMBOL_NAME(
    rosidl_typesupport_c,
    dwb_msgs,
    srv,
    GenerateTwists
  ),
  ROSIDL_TYPESUPPORT_INTERFACE__SERVICE_DESTROY_EVENT_MESSAGE_SYMBOL_NAME(
    rosidl_typesupport_c,
    dwb_msgs,
    srv,
    GenerateTwists
  ),
  &dwb_msgs__srv__GenerateTwists__get_type_hash,
  &dwb_msgs__srv__GenerateTwists__get_type_description,
  &dwb_msgs__srv__GenerateTwists__get_type_description_sources,
};

}  // namespace rosidl_typesupport_c

}  // namespace srv

}  // namespace dwb_msgs

#ifdef __cplusplus
extern "C"
{
#endif

const rosidl_service_type_support_t *
ROSIDL_TYPESUPPORT_INTERFACE__SERVICE_SYMBOL_NAME(rosidl_typesupport_c, dwb_msgs, srv, GenerateTwists)() {
  return &::dwb_msgs::srv::rosidl_typesupport_c::GenerateTwists_service_type_support_handle;
}

#ifdef __cplusplus
}
#endif
